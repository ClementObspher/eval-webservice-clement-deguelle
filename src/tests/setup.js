...
async function getKeycloakToken() {
	const res = await request(process.env.KEYCLOAK_URL).post(`/realms/${process.env.KEYCLOAK_REALM}/protocol/openid-connect/token`).type("form").send({
		grant_type: "password",
...
async function getKeycloakAdminToken() {
	const res = await request(process.env.KEYCLOAK_URL).post(`/realms/master/protocol/openid-connect/token`).type("form").send({
		grant_type: "password",
		client_id: "admin-cli",
