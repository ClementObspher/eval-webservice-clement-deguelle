...
describe("Reservations E2E Tests", () => {
	...
	beforeAll(async () => {
		...
		console.log("Utilisateurs trouvés:", responseUsers.data.users)
    ...
    `
		...
		console.log("Réponse création réservation:", data)
	...
	it("should get the created reservation by ID in database", async () => {
		const pool = getPool()
		const { rows } = await pool.query(`SELECT * FROM reservations WHERE id = $1`, [createdReservationId])
	...
	it("should find a notification in table notifications with this reservation id", async () => {
		const pool = getPool()
		const { rows } = await pool.query(`SELECT * FROM notifications WHERE "reservation_id" = $1`, [createdReservationId])
		expect(rows.length).toBe(1)
		await closePool()
	})
    ...
    `
		...
		const found = data.listReservations.find((r) => r.id === createdReservationId)
